#!/usr/bin/env python3
"""Discover process models from an event log."""
...
from process_mining_steps import discover_models, evaluate_models, load_event_log, save_models


def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Process discovery for event logs.")
...
def main() -> None:
    ...
    stage_dir = ensure_stage_dir(args.output, "stage_05_discover")
    try:
        ...
        model_artifacts = save_models(models, stage_dir)
        notebook_path = ensure_notebook(
            ...
            context_lines=[
                "",
                f"- Miner selection: {args.miner_selection}",
                f"- Noise threshold: {args.noise_threshold}",
            ],
            code_lines=[
                "import pandas as pd",
        ...
        if model_artifacts:
            artifacts["models_manifest_json"] = f"{stage_dir}/models_manifest.json"
        ...
        record_stage_failure(
            ...
            [
                ...
                "Adjust miner thresholds if models fail to discover.",
                "Re-run discovery after corrections.",
            ],
        )
